...
#define	 immagine_in_scrittura_ppm	"../image/img4.ppm"       /* immagine in scrittura  in forma pgm */
#define  immagine_in_scrittura          "../image/img4.txt"           /* immagine in scrittura in forma matriciale */
...
double immagine_temp_rossa[M][N], immagine_temp_verde[M][N], immagine_temp_blu[M][N];
...
main()
{
  ...
  leggi();                  // lettura dai dati dal file
    ...
    for(i = 0; i < M; i++){
      for (j = 0; j < N; j++){
	immagine_temp_rossa[i][j] = immagine_originale_rossa[i][j];
	immagine_temp_verde[i][j] = immagine_originale_verde[i][j];
	immagine_temp_blu[i][j] = immagine_originale_blu[i][j];
      }
    }
	...
	{
	  immagine_temp_rossa[i][j] = 255;
	  immagine_temp_verde[i][j] = 255;
	  immagine_temp_blu[i][j] = 255;
	}
	...
	if((j - 85) * (j - 85) + (i - 35) * (i - 35) <= 700){
	  immagine_temp_verde[i][j] = 0;
	}
	else if((j - 165) * (j - 165) + (i - 35) * (i - 35) <= 700){
	  immagine_temp_blu[i][j] = 0;
	}
      }
    }
    ...
    //disegno riga centrale
    i = M/2;
    for(j = 0; j < N; j++){
      immagine_temp_rossa[i][j] = 0;
      immagine_temp_verde[i][j] = 0;
      immagine_temp_blu[i][j] = 0;
    }
    ...
    for(i = (M / 2) + 1; i < M; i++){
      for (j = 0; j < N; j++){
	immagine_temp_rossa[i][j] = immagine_temp_rossa[M - i][N - j];
	immagine_temp_verde[i][j] = immagine_temp_verde[M - i][N - j];
	immagine_temp_blu[i][j] = immagine_temp_blu[M - i][N - j];
      }
    }
    
    //scrivo tutto su finale
    for(i = 0; i < M; i++){
      for (j = 0; j < N; j++){
	immagine_degradata_rossa[i][j] = immagine_temp_rossa[i][j];
	immagine_degradata_verde[i][j] = immagine_temp_verde[i][j];
	immagine_degradata_blu[i][j] = immagine_temp_blu[i][j];
      }
    }
